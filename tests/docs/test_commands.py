...
def test_cuda_dev_compose_uses_dev_cuda_image() -> None:
    compose = yaml.safe_load(Path("compose.cuda.dev.yml").read_text(encoding="utf-8"))
    service = compose["services"]["essentia-studio"]
    assert service["image"].endswith(":dev-cuda")
