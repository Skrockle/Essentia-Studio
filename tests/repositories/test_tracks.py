...
def test_query_all_reads_the_ordered_match_set_without_pagination(tmp_path, monkeypatch) -> None:
    engine = create_sqlite_engine(tmp_path / "app.db")
    apply_migrations(engine)
    repository = TrackRepository(engine)
    repository.replace_scan(
    ...
    def paginated_query_is_not_allowed(*_args, **_kwargs) -> None:
        raise AssertionError("query_all must not use paginated query")

    monkeypatch.setattr(repository, "query", paginated_query_is_not_allowed)

    selected = repository.query_all(LibraryQuery())
