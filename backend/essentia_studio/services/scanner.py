...
from essentia_studio.services.managed_tag_inventory import read_managed_tag_inventory
...
def scan_music_root(
    ...
    tag_registry: TagAdapterRegistry | None = None,
) -> Iterator[ScannedTrack]:
    resolved_root = root.resolve(strict=True)
    reader = metadata_service or MetadataService()
    registry = tag_registry or TagAdapterRegistry()
    ...
    for path in resolved_root.rglob("*"):
        ...
        scanned_tracks.append(
            ScannedTrack(
                ...
                managed_tags=read_managed_tag_inventory(path, registry),
            )
        )
